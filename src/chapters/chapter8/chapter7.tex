...
% Second, our multi-scale analysis across different electoral hierarchies—from individual polling booths (~$10^2$ voters) to parliamentary constituencies (~$10^6$ voters)—provides a robust validation framework for theoretical predictions. Particularly noteworthy is our discovery of scale invariance in Indian margin distributions, where $Q_M(M̃)$ shows a remarkable data collapse across different electoral scales. This methodological approach of cross-scale validation strengthens confidence in the RVM's theoretical foundations.
